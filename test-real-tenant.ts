...
const TEST_WHITELIST: SortFieldWhitelist = ["id", "email", "created_at"] as const;
...
setTimeout(() => {
  ...
  process.stdout.write("SELECT * FROM adm_audit_logs WHERE tenant_id = '" + REAL_TENANT_ID + "'\n");
  process.stdout.write("AND action = 'validation_failed' ORDER BY timestamp DESC LIMIT 2;\n");
  process.exit(0);
}, 2000);
