...
/**
 * Hook pour gérer les préférences de colonnes Opportunities avec persistance localStorage
 * SSR-safe avec pattern mounted state
 * Basé sur useColumnPreferences.ts (Leads) - même pattern pour cohérence UX
 */
...
import {
  DEFAULT_OPPORTUNITY_COLUMN_ORDER,
  DEFAULT_OPPORTUNITY_COLUMN_WIDTHS,
} from "@/lib/config/opportunity-columns";

const STORAGE_KEY = "crm_opportunities_columns";
...
export function useOpportunityColumnPreferences(
  defaultColumns: OpportunityColumnConfig[]
) {
  const [mounted, setMounted] = useState(false);
  const [columns, setColumns] =
    useState<OpportunityColumnConfig[]>(defaultColumns);
  ...
  const resetToDefault = useCallback(() => {
    ...
    setColumnOrder(DEFAULT_OPPORTUNITY_COLUMN_ORDER);
    setColumnWidths(DEFAULT_OPPORTUNITY_COLUMN_WIDTHS);
  }, []);
  ...
  const getColumnWidth = useCallback(
    (key: string): number => {
      return columnWidths[key] ?? DEFAULT_OPPORTUNITY_COLUMN_WIDTHS[key] ?? 100;
    },
    [columnWidths]
  );
