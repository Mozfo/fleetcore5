...
export class WizardLeadService {
  ...
  async createWizardLead(
    ...
    // V7: Resolve tenant from country (operational → mapped tenant, else → expansion)
    const tenantId = await resolveTenantByCountry(normalizedCountryCode);
   ...
   * Also transitions status from "new" → "email_verified" (V7 pipeline).
   * Only promotes — never downgrades if lead already has an advanced status.
   *
   * @param leadId - Lead UUID
   */
  async markEmailVerified(leadId: string): Promise<void> {
    // Read current status to avoid downgrading advanced leads
