-- ============================================================================
-- SPRINT 3 FINAL - OPTIMIZE 47 RLS POLICIES (auth_rls_initplan)
-- ============================================================================
-- Date: 2025-11-23
-- Issue: Supabase lint rule 0003_auth_rls_initplan
-- Problem: current_setting() re-evaluated for EACH row
-- Solution: Wrap in (SELECT ...) to evaluate ONCE per query
-- Expected gain: 70-90% performance improvement
-- ============================================================================
-- TESTED: Syntax verified on adm_members - WORKS!
-- Backup: backup_47_policies_REAL_20251123_160045.sql (18KB)
-- ============================================================================
...
-- MODULE: RIDE-HAILING (7 standard policies)
...
-- SPECIAL: rid_driver_languages (3 policies with COALESCE + deleted_at)
...
USING ((tenant_id)::text = COALESCE((SELECT current_setting('app.current_tenant_id', true)), '') AND deleted_at IS NULL);
...
USING ((tenant_id)::text = COALESCE((SELECT current_setting('app.current_tenant_id', true)), '') AND deleted_at IS NULL)
WITH CHECK ((tenant_id)::text = COALESCE((SELECT current_setting('app.current_tenant_id', true)), ''));
...
-- MODULE: SUPPORT (3 policies - 2 standard + 1 EXISTS)
...
-- SPECIAL: sup_ticket_messages (EXISTS subquery)
...
USING (EXISTS (
  ...
  WHERE t.id = sup_ticket_messages.ticket_id
    AND t.tenant_id::text = (SELECT current_setting('app.current_tenant_id', true))
))
WITH CHECK (EXISTS (
  SELECT 1 FROM sup_tickets t
  WHERE t.id = sup_ticket_messages.ticket_id
...
-- MODULE: DIRECTORY (2 policies - shared catalog)
...
FOR ALL TO authenticated
USING (tenant_id IS NULL OR tenant_id::text = (SELECT current_setting('app.current_tenant_id', true)));
...
-- END OF MIGRATION
-- ============================================================================
-- Summary: 47 policies optimized
-- Syntax: tenant_id::text = (SELECT current_setting(...))
-- Tested: adm_members verified working
-- Next: Execute verify_auth_initplan_47_policies_REAL.sql
-- ============================================================================
