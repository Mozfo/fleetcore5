...
sequenceDiagram
    ...
    Note over Service: STEP 5: Create Lead
    Service->>DB: INSERT crm_leads
    Note right of DB: Trigger: trg_set_lead_code<br/>→ generate_entity_code('L-')<br/>→ lead_code = "L-R7M8J6"
    DB-->>Service: lead record (with lead_code)

    Note over Service: STEP 6: Notify
...
The `lead_code` is automatically generated by a PostgreSQL trigger when a lead is inserted. This ensures:

- **Uniqueness:** The trigger retries up to 10 times if a collision is detected
- **Security:** The code is random and does not reveal business volume (no sequential numbers)
- **Readability:** Uses 32 unambiguous characters (excludes I, O, L to avoid confusion with 1, 0, l)

**Format:** `L-XXXXXX` (prefix + 6 random alphanumeric characters)

**Examples:** `L-R7M8J6`, `L-CQDM56`, `L-PVN7SG`

**Database Components:**

| Component                    | Purpose                                          |
| ---------------------------- | ------------------------------------------------ |
| `trg_set_lead_code`          | Trigger on INSERT, fires `set_lead_code()`       |
| `set_lead_code()`            | Checks if lead_code is null, generates if needed |
| `generate_entity_code('L-')` | Generates random code with prefix                |

**Character Set:** `0123456789ABCDEFGHJKMNPQRSTUVWXYZ` (32 chars)

**Note:** The same `generate_entity_code()` function is used for other entity codes:

- Tenant code: `C-XXXXXX`
- Lead code: `L-XXXXXX`
...
flowchart TB
    subgraph System["🤖 System (Automated)"]
        ...
        S5[Generate Lead Code<br/>PostgreSQL Trigger]
    end
...
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| **System**    | Lead code generation (PostgreSQL trigger), score calculation, automatic assignment, rotting detection, notification dispatch, audit logging |
