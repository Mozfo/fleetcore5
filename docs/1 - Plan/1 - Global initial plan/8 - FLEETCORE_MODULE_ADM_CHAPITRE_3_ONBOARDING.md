...
2. Préfixer : token = "inv\_" + base_uuid
3. Vérifier unicité dans adm_invitations.token
4. SI existe déjà (collision rare) :
   ALORS régénérer récursivement
5. Valider format : /^inv\_[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/
...
Conditions : - status = pending - sent_at + 48h < now - metadata.reminder_sent_at IS NULL
Action : - Envoyer email "invitation_reminder" - metadata.reminder_sent_at = now

RÈGLE RELANCE 2 : +24h avant expiration
Conditions : - status = pending - expires_at - 24h < now - metadata.final_reminder_sent_at IS NULL
Action : - Envoyer email "invitation_expiring_soon" - metadata.final_reminder_sent_at = now
